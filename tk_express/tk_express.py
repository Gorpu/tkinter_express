...
class TkExpress():
    ...
    def TextLabel(context: tk.Tk, title: str, row: int, column: int, color_bg="#ffffff"):
        labelComp = tk.Label(
        context, 
        bg=color_bg, 
        fg="#FFFFFF",
        text=title)
        labelComp.grid(row=row, column=column)
    def CheckBox(context: tk.Tk, title: str,row: int, column:int, value: tk.BooleanVar, color_bg="#FFFFFF"):
        def pressed():
            print(value.get())
    ...
    def InputText(context: tk.Tk, width:int, row: int, column:int, size_return=25):
    ...
    def ActionButton(context: tk.Tk, text: str, command: callable, row: int, column: int):
        action_button = tk.Button(context, text=text, command=command)
        action_button.grid(row=row, column=column)
    
    def Dialog(mensage: str, title="Alert", dialog_error=False,geometry="250x120", resizable=False, command=None, button_title="Ok"):
        icon_win = add_bin("domain/lib/tkExpress/assets/erro_icon_win.ico")  if dialog_error else add_bin("domain/lib/tkExpress/assets/sucess_icon_win.ico")
